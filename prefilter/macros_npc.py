...
# ------------------------------------------------------------------------
            ...
            if (True):
                print("\n(score,\t\t rmf_file,\t\t\t rmf_frame_number, raw index, rank)")
                for i in range(len(my_best_score_rmf_tuples)):
                    print (my_best_score_rmf_tuples[i])
                    #for j in range(len(my_best_score_rmf_tuples[0])):
                    #    print (my_best_score_rmf_tuples[i][j])
...
# ------------------------------------------------------------------------
            if skip_clustering:
                ...
                for cnt,tpl in enumerate(my_best_score_rmf_tuples):
                    ...
                    rmf_slice_command = "rmf_slice " + rmf_name + " " + out_rmf_fn + " -f " + str(rmf_frame_number) + " -s 1000000"
                    print(rmf_slice_command)
                    os.system(rmf_slice_command)

                    statfile = rmf_name.replace("/rmfs//", "/stat.")
                    statfile = statfile.replace(".rmf3", ".out")
                    statindex = statfile.split(".")[1]
                    process_output_command = "process_output.py -f " + statfile + " -n " + str(rmf_frame_number+1) + " > " + str(cnt)+"_"+str(statindex)+"_"+str(rmf_frame_number+1) + ".log"
                    print(process_output_command)
                    os.system(process_output_command)

                    # for the whole NPC - skip the rest, due to a limited range of chain IDs in the PDB format.
                    if (False):
                        continue

                    try:
                        prots,rs = IMP.pmi1.analysis.get_hiers_and_restraints_from_rmf(self.model, 0, out_rmf_fn)
                    except:
                        continue
                    ...
                    try:
                        o.init_pdb(out_pdb_fn, prot)
                        o.write_pdb(out_pdb_fn, translate_to_geometric_center=write_pdb_with_centered_coordinates)
                    except:
                        continue
