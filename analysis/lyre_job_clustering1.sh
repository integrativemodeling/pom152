...
for (( SGE_TASK_ID=1; SGE_TASK_ID >= 1; SGE_TASK_ID-- )); do
    echo "NSLOTS = $NSLOTS"
    echo "JOB_ID = $JOB_ID"
    ...
    #run
    echo "$IMP python ./clustering.py -mpi False -preload True -nmods $NMODS -nclusters $SGE_TASK_ID -prefilter $PREFILTER"; echo
