...
def score_model(rmffile, image_file, pixel_size, n_projections, resolution, image_resolution, frame_number):
    ...
    for p in particle2s:
        ...
        if p.get_parent().get_name() == "Beads" and p.get_name()[0:6] == "Dyn2.1": # and p.get_parent().get_name()[0:6] == "Dyn2.1":
            ...
            ps.append(p)
        elif p.get_parent().get_name() == "Beads" and p.get_name()[0:3] == "656":
            ps.append(p)
        elif p.get_parent().get_name() == "Beads" and p.get_name()[0:3] == "742":
            ps.append(p)
        """
        ps.append(p)
    ...
    print ("len(ps) = ", len(ps))       #len(particles_all)=2574

    if (False):
        # writing the particle densities
        map = IMP.em.SampledDensityMap(ps, resolution, resolution)
